...
const Header = () => {
  ...
  const desktopMode = useMediaQuery({ // Nos indica si estamos por encima ((true) -> desplaz. suave) o no (false -> desplaz.rápido) de los 1300px
    query: '(min-width: 1300px)'
  });

  useEffect(() => {

    const handleScroll = () => {                        // Definimos la función que manejará el evento de desplazamiento

      if (window.scrollY > 40) {                        // Comprobamos si el valor de desplazamiento vertical es mayor que 40px
  ...
  return (
    <div className={`${header ? 'bg-white shadow-md py-2' : 'bg-transparent shadow-none py-4'} fixed w-full max-w-[1920px] mx-auto z-20
    transition-all duration-300`}>
      <div className='xl:container mx-auto flex flex-col xl:flex-row xl:items-center xl:justify-between'>
        ...
        <div className='flex justify-between items-center px-4'>
          ...
          <Link 
            to='home' 
            smooth={desktopMode}  // smooth habilita o no el desplazamiento suave al enlace del link
            spy={true}            // Hace que el link aparezca como seleccionado cuando el target del link se ha alcanzado
